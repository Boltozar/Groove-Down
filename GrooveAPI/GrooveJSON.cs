...
namespace GrooveAPI
{
	class GrooveJSON
	{
		public void WriteHeader(string methodForToken = null, bool downloading = false)
		{
			...
			{
				_writer.WriteMember("token", GenerateToken(methodForToken, downloading));
			}
      ...
      _writer.WriteMember("IPR", "377");
		...
		private string GenerateToken(string method, bool isDownload = false)
		{
			const string salt = "f4001b";

			StringBuilder enc = new StringBuilder(salt);

			enc.Append(Helpers.Hash(method + ":" + Information.Token + ":" + (!isDownload ? Information.GrooveAPISalt : 
        Information.GrooveAPISaltDownload) + ":" + salt, new SHA1CryptoServiceProvider()));
